...
import { collection, query, where, limit, getDocs, startAfter, orderBy } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
async function handleSearch(loadMore = false) {
    ...
    try {
        ...
        if (snapshot.empty) {
            if (!loadMore) resultsContainer.innerHTML = '<p style="text-align:center; color:#666;">No results found.</p>';
...
function renderCarSearchCard(id, data) {
    ...
    const minDiff = Math.min(
        Math.ceil((data.licenseExpiry.toDate() - today) / (1000*60*60*24)),
        Math.ceil((data.insuranceExpiry.toDate() - today) / (1000*60*60*24))
    );
    ...
    card.innerHTML = `
        <div class="card-header" onclick="this.parentElement.classList.toggle('open')">
            <span class="card-title">${data.plateNumber} ${data.plateCode} (${data.emirate})</span>
...
function renderLogSearchCard(id, data) {
    ...
    const card = document.createElement('div');
    card.className = 'card border-blue';
    let dateStr = data.timestamp ? new Date(data.timestamp.toDate()).toLocaleString('en-GB') : 'N/A';
    card.innerHTML = `
        <div class="card-header" onclick="this.parentElement.classList.toggle('open')">
        ...
        <div class="card-body">
            <div class="detail-list">
                ...
                <div class="detail-item"><span class="detail-label">Details</span><span class="detail-value">${data.details}</span></div>
            </div>
        </div>
    `;
    list.appendChild(card);
