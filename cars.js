...
import {
    ...
    setLoading, appendLoading, removeLoading,
    disableLoadMoreButton, enableLoadMoreButton, UI_TEXTS
} from "./loadingManager.js";
import {
    formatDateTime, formatDateOnly, formatPeriod, formatCarLabel,
    isAdmin, isActiveUser, renderAccessDenied,
...
export function renderCarsView() {
    ...
    if (!container) return;
    if (!isActiveUser(currentUserData)) { renderAccessDenied(); return; }
    ...
    if (isAdmin(currentUserData)) {
        container.innerHTML = `
            ...
            <div id="add-car-form-wrapper" class="hidden-form" style="margin-bottom: 30px;">
                <form id="add-car-form" style="display:grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div class="form-group"><label>Plate Number (Digits)</label><input type="text" id="car-plate-num" required pattern="\\d+" maxlength="6" placeholder="e.g. 12345"></div>
                    <div class="form-group"><label>Plate Code</label><input type="text" id="car-plate-code" required maxlength="3" placeholder="e.g. A"></div>
                    <div class="form-group"><label>Emirate</label><select id="car-emirate" required>
                        <option value="Abu Dhabi">Abu Dhabi</option><option value="Dubai">Dubai</option>
                        <option value="Sharjah">Sharjah</option><option value="Ajman">Ajman</option>
                        <option value="Fujairah">Fujairah</option><option value="Umm Al Quwain">Umm Al Quwain</option>
                        <option value="Ras Al Khaimah">Ras Al Khaimah</option><option value="Other">Other</option>
                    </select></div>
                    <div class="form-group"><label>Type (Make)</label><input type="text" id="car-type" required placeholder="e.g. Toyota Camry"></div>
                    <div class="form-group"><label>Owner Name</label><input type="text" id="car-owner" required></div>
                    <div class="form-group"><label>VIN (Unique)</label><input type="text" id="car-vin" required maxlength="17"></div>
        ...
        if (toggleAddCar) toggleAddCar.addEventListener('click', () => {
        ...
        container.innerHTML = `
            ...
            <div id="request-car-form-wrapper" class="hidden-form" style="margin-bottom: 30px;">
                <form id="request-car-form" style="display:grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div class="form-group"><label>Plate Number</label><input type="text" id="req-plate-num" required pattern="\\d+" maxlength="6"></div>
                    <div class="form-group"><label>Plate Code</label><input type="text" id="req-plate-code" required maxlength="3"></div>
                    <div class="form-group"><label>Emirate</label><select id="req-emirate" required>
                        <option value="Abu Dhabi">Abu Dhabi</option><option value="Dubai">Dubai</option>
                        <option value="Sharjah">Sharjah</option><option value="Ajman">Ajman</option>
                        <option value="Fujairah">Fujairah</option><option value="Umm Al Quwain">Umm Al Quwain</option>
                        <option value="Ras Al Khaimah">Ras Al Khaimah</option><option value="Other">Other</option>
        ...
        if (toggleRequestCar) toggleRequestCar.addEventListener('click', () => {
            const wrapper = document.getElementById('request-car-form-wrapper');
            if (wrapper) wrapper.classList.toggle('hidden-form');
        });
        if (requestCarForm) requestCarForm.addEventListener('submit', createLinkRequest);
        fetchUserCars();
    }
}
...
async function handleAddCar(e) {
    ...
    try {
        ...
        await setDoc(doc(db, 'cars', carId), {
            carId, plateNumber: plateNum, plateCode, emirate, plateIdentifier,
            type, ownerName: owner, vin,
            licenseExpiry: new Date(licenseExp), insuranceExpiry: new Date(insuranceExp),
            notes, currentUserId: null, currentUserName: null, status: 'active', createdAt: serverTimestamp()
        });
        ...
        showDashboardMessage('Success: Car added successfully.', 'success');
    ...
    try {
        ...
        if (loadMoreContainer) {
            if (snapshot.size === 10) {
                ...
                if (newBtn) newBtn.addEventListener('click', () => fetchCars(true));
            } else {
...
function renderCarCard(id, data, isUserView = false) {
    ...
    const emirateColors = {
        'Abu Dhabi': '#0070c0', 'Dubai': '#b91d1d', 'Sharjah': '#000000',
        'Ajman': '#ed1c24', 'Fujairah': '#8a2be2', 'Umm Al Quwain': '#006400',
        'Ras Al Khaimah': '#ff8c00', 'Other': '#666666'
    };
    ...
    if (!isUserView) {
        actionsHtml = `
            <div class="action-buttons" id="car-actions-${id}">
                ...
                ${data.currentUserId ? '<button type="button" class="action-btn action-btn-unassign" data-action="unassign">👤 Unassign</button>' : '<button type="button" class="action-btn action-btn-assign" data-action="assign">👤 Assign</button>'}
    ...
    card.innerHTML = `
        ...
        <div class="card-body" id="body-${id}">
            <div class="detail-list">
                ...
                <div class="detail-item"><span class="detail-label">License Expiry</span><span class="detail-value ${licClass}">${formatDateOnly(data.licenseExpiry)} (${licDiff} days left)</span></div>
                <div class="detail-item"><span class="detail-label">Insurance Expiry</span><span class="detail-value ${insClass}">${formatDateOnly(data.insuranceExpiry)} (${insDiff} days left)</span></div>
...
async function handleCarAction(id, action, data, topBarColor) {
    ...
    if (action === 'edit') renderEditCarForm(id, data);
    else if (action === 'print') handlePrintCard(data, topBarColor);
    else if (action === 'history') await renderCarHistory(id, data);
...
function renderEditCarForm(carId, data) {
    ...
    editArea.innerHTML = `
        ...
        <form id="edit-car-form-${carId}" class="edit-car-form">
            <div class="form-group"><label>Plate Number</label><input type="text" id="edit-plate-num-${carId}" value="${data.plateNumber}" required pattern="\\d+" maxlength="6"></div>
            <div class="form-group"><label>Plate Code</label><input type="text" id="edit-plate-code-${carId}" value="${data.plateCode}" required maxlength="3"></div>
            <div class="form-group"><label>Emirate</label><select id="edit-emirate-${carId}" required>
            ...
            <div class="form-group"><label>Type (Make)</label><input type="text" id="edit-type-${carId}" value="${data.type}" required></div>
            <div class="form-group"><label>Owner Name</label><input type="text" id="edit-owner-${carId}" value="${data.ownerName}" required></div>
            <div class="form-group"><label>VIN</label><input type="text" id="edit-vin-${carId}" value="${data.vin}" required maxlength="17"></div>
            <div class="form-group"><label>License Expiry</label><input type="date" id="edit-lic-${carId}" value="${toDateInputValue(data.licenseExpiry)}" required></div>
            <div class="form-group"><label>Insurance Expiry</label><input type="date" id="edit-ins-${carId}" value="${toDateInputValue(data.insuranceExpiry)}" required></div>
            <div class="form-group full-width"><label>Notes</label><input type="text" id="edit-notes-${carId}" value="${data.notes || ''}"></div>
    ...
    if (editForm) editForm.addEventListener('submit', async (e) => { e.preventDefault(); await handleSaveEditCar(carId, data); });
    const cancelEditBtn = document.getElementById(`cancel-edit-${carId}`);
    if (cancelEditBtn) cancelEditBtn.addEventListener('click', () => { editArea.style.display = 'none'; editArea.innerHTML = ''; });
}
...
async function handleSaveEditCar(carId, originalData) {
    ...
    try {
        ...
        await updateDoc(doc(db, 'cars', carId), {
            plateNumber: plateNum, plateCode, emirate, plateIdentifier,
            type, ownerName: owner, vin,
        ...
        await logAction(currentUserData, 'EDIT_CAR', { targetId: carId, targetName: label, text: `Edited car ${label}` });
        showDashboardMessage('Success: Car updated successfully.', 'success');
        lastVisibleCar = null;
        fetchCars(false);
    } catch (error) {
        showDashboardMessage(`Error: ${error.message}`, 'error');
...
<html>
<head><meta charset="UTF-8"><title>Car Card - ${safe(data.carId)}</title>
<style>
@page { margin: 12mm; }
...
<body>
    <div class="print-header"><h2>Car Management System</h2></div>
    <h3>${safe(label)}</h3>
    <div class="plate-container">
        <div style="display:flex; flex-direction:column; align-items:center;">
    ...
    <div class="details">
        ...
        idoc.close();
        const triggerPrint = () => {
            try { iframe.contentWindow.focus(); iframe.contentWindow.print(); } catch (_) { showDashboardMessage('Error: Unable to open print dialog.', 'error'); }
        ...
        else { iframe.onload = () => setTimeout(triggerPrint, 200); setTimeout(triggerPrint, 500); }
    } catch (error) { showDashboardMessage(`Print error: ${error.message}`, 'error'); }
}
...
async function renderCarHistory(carId, carData) {
    ...
    try {
        ...
        if (logsSnap.empty) html += '<p class="history-item">No activity recorded yet.</p>';
        else {
            const logs = [];
            ...
            logs.slice(0, 8).forEach(log => {
                ...
                html += `<div class="history-item"><span class="action-type">${log.actionType}</span> by ${log.actorName}<br>${log.details || ''}<br><span class="timestamp-meta">${date}</span></div>`;
            });
        }
        const assignQuery = query(collection(db, 'cars', carId, 'assignments'), orderBy('startTime', 'desc'), limit(5));
        ...
        if (!assignSnap.empty) {
            ...
            assignSnap.forEach(doc => {
                ...
                html += `<div class="history-item"><strong>${a.userName}</strong><br>${period}</div>`;
            });
        }
        html += '</div>';
...
async function renderAssignUserUI(carId) {
    ...
    try {
        ...
        snapshot.forEach(d => { const u = d.data(); options += `<option value="${d.id}">${u.username} (${u.role})</option>`; });
        ...
        confirmAssignBtn.addEventListener('click', async () => {
            ...
            if (!userId) { showDashboardMessage('Please select a user first.', 'warning'); return; }
            try {
                ...
                await logAction(currentUserData, 'CAR_ASSIGN', { targetId: carId, targetName: label, assigneeId: userId, text: `Assigned ${label} to ${userName}` });
                showDashboardMessage('User assigned successfully.', 'success');
                lastVisibleCar = null;
                fetchCars(false);
            } catch (err) { showDashboardMessage(`Error: ${err.message}`, 'error'); }
        });
...
async function handleUnassignUser(carId, data) {
    ...
    try {
        ...
        if (!snap.empty) await updateDoc(doc(db, 'cars', carId, 'assignments', snap.docs[0].id), { endTime: serverTimestamp() });
        await logAction(currentUserData, 'CAR_UNASSIGN', { targetId: carId, targetName: label, assigneeId: data.currentUserId, text: `Unassigned ${label} from ${data.currentUserName}` });
        showDashboardMessage('User unassigned successfully.', 'success');
        lastVisibleCar = null;
        fetchCars(false);
    } catch (err) { showDashboardMessage(`Error: ${err.message}`, 'error'); }
}